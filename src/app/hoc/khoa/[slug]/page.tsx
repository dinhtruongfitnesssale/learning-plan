...
import { enroll } from "../../khoa-hoc/actions";
...
export default async function CoursePage({
  ...
  const { course, lessons, enrolled, done, total, leaderboard } = data;
  const percent = total ? done / total : 0;

  // Nhóm bài theo module (giữ thứ tự xuất hiện).
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        {!enrolled && (
          <form action={enroll}>
            ...
            <button className={buttonClass("primary")}>Ghi danh học</button>
          </form>
        )}
      </header>
      ...
      <div className="grid lg:grid-cols-[1fr_280px] gap-8 items-start">
                ...
                {g.lessons.map(({ lesson, done, hasQuiz }, i) => (
                  <li key={lesson.id}>
                    <Link href={`/hoc/khoa/${course.slug}/${lesson.slug}`}>
                      <Card className="px-4 py-3.5 flex items-center gap-4 hover:border-ink/25 transition-colors">
                        <span
        ...
        {/* Bảng xếp hạng tuần (micro-competition) */}
        <aside>
          <Card className="p-5 sticky top-20">
            ...
            {leaderboard.length === 0 ? (
              <p className="text-sm text-ink/50">Chưa có ai. Học bài để dẫn đầu!</p>
            ) : (
